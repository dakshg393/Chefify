...
#         try:
#             rating_instance = RecipeRating.objects.get(user=request.user, recipe=recipe)
#         except RecipeRating.DoesNotExist:
#             rating_instance = None

#         return render(request, 'recipe_detail.html', {'recipe': recipe, 'recipe_detail': recipe_detail, 'recipe_steps': recipe_steps , 'ingredients_list': ingredients_list ,'rating_instance':rating_instance})
...
def recipe_detail(request, pk):
    ...
    recipe_detail = get_object_or_404(RecipeDetails, recipeid=pk)
    recipe_steps = get_object_or_404(RecipeSteps, recipeid=pk)
    ingredients_list = recipe_detail.ingredients.split(',')
    ...
    if request.method == 'POST':
        form = RecipeRatingForm(request.POST, instance=rating_instance)
        if form.is_valid():
        ...
        form = RecipeRatingForm(instance=rating_instance)

    ratings_list = RecipeRating.objects.filter(recipe=recipe).order_by('-created_at')
    paginator = Paginator(ratings_list, 4)  # Show 4 ratings per page
